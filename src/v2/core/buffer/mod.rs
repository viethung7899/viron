...
use crate::{
    core::buffer::gap_buffer::GapBuffer,
    editor::Mode,
    service::lsp::{Diagnostic, TextDocumentPublishDiagnostics},
    utils,
};
...
impl Buffer {
    ...
    pub fn delete_char(&mut self, cursor: &mut Point, mode: &Mode) -> Option<char> {
        self.dirty = true;
        let position = self.cursor_position(cursor);
        self.buffer.move_gap(position);
