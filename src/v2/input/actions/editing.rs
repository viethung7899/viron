...
impl Action for InsertChar {
    fn execute(&self, ctx: &mut ActionContext) -> ActionResult {
        let position = ctx.buffer.cursor_position(&ctx.cursor.get_position());
        let new_position = ctx.buffer.insert_char(position, self.0);
        ctx.cursor
            .set_position(ctx.buffer.point_at_position(new_position));
        Ok(())
    }
...
impl Action for DeleteChar {
    fn execute(&self, ctx: &mut ActionContext) -> ActionResult {
        let position = ctx.buffer.cursor_position(&ctx.cursor.get_position());
        if let Some(_) = ctx.buffer.delete_char(position) {
...
impl Action for Backspace {
    fn execute(&self, ctx: &mut ActionContext) -> ActionResult {
        let position = ctx.buffer.cursor_position(&ctx.cursor.get_position());
        if position > 0 {
            if let Some(_) = ctx.buffer.delete_char(position - 1) {
...
impl Action for InsertNewLine {
    fn execute(&self, ctx: &mut ActionContext) -> ActionResult {
        let position = ctx.buffer.cursor_position(&ctx.cursor.get_position());
        let new_position = ctx.buffer.insert_char(position, '\n');
        ctx.cursor
            .set_position(ctx.buffer.point_at_position(new_position));
        Ok(())
    }
